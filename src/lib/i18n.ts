...
const dictionaries: Record<Locale, Dictionary> = {
  en: {
    ...
    subtitle: "Time from filing to decision or dismissal across Danish rent boards.",
    ...
    chartHeading: "Average days from filing to decision or dismissal, by municipality",
    chartTab: "Chart",
    tableTab: "Table",
    sortLabel: "Sort",
    ...
    daysAxis: "Days",
    trendHeading: "Average days from filing to decision or dismissal, over time",
    trendYearAxis: "Year",
    ...
    exclusionMissingDatesNote:
      "Cases missing both a dateOfDecision and a dateOfDismissal (or, in principle, a dateOfFiling) entirely are never synced into this database at all, so they aren't part of the count above either — those cases fall outside this filtered view completely.",
    ...
    caseStatusMethodLinesDecision: [
      "Bucketed by year of dateOfDecision (decision date) instead — dismissed cases never have one, so those specifically fall back to dateOfDismissal; every other status still uses dateOfDecision.",
      "Not-yet-decided cases have neither a dateOfDecision nor a dateOfDismissal (the case is still open), so that series reads close to zero in every year here — that's expected, not a data error.",
      "A small number of cases with an actual outcome (in favour, rejected, settled, partially in favour) also lack a dateOfDecision and aren't rescued by a fallback the way dismissed cases are, so they don't appear in any year in this view — unlike the filing-date view, this one does not cover every case in the source system.",
    ],
    ...
    quarterlyStatusMethodLinesDecision: [
      "Bucketed by quarter of dateOfDecision (decision date) instead — dismissed cases never have one, so those specifically fall back to dateOfDismissal; every other status still uses dateOfDecision.",
      "Not-yet-decided cases have neither a dateOfDecision nor a dateOfDismissal (the case is still open), so that column reads close to zero in every quarter here — that's expected, not a data error.",
      "A small number of cases with an actual outcome (in favour, rejected, settled, partially in favour) also lack a dateOfDecision and aren't rescued by a fallback the way dismissed cases are, so they don't appear in any quarter in this view — unlike the filing-date view, this one does not cover every case in the source system.",
    ],
  ...
  da: {
    ...
    subtitle: "Sagsbehandlingstid fra klage til afgørelse eller henlæggelse hos de danske huslejenævn.",
    ...
    chartHeading: "Gennemsnitligt antal dage fra indbringelse til afgørelse eller henlæggelse, pr. kommune",
    chartTab: "Diagram",
    tableTab: "Tabel",
    sortLabel: "Sortér",
    ...
    daysAxis: "Dage",
    trendHeading: "Gennemsnitligt antal dage fra indbringelse til afgørelse eller henlæggelse, over tid",
    trendYearAxis: "År",
    ...
    exclusionMissingDatesNote:
      "Sager helt uden både en afgørelsesdato og en henlæggelsesdato (eller i princippet en indbringelsesdato) bliver aldrig synkroniseret til denne database, så de er heller ikke en del af tallet ovenfor — de falder helt uden for denne filtrerede visning.",
    ...
    caseStatusMethodLinesDecision: [
      "Opdelt efter år for dateOfDecision (afgørelsesdato) i stedet — henlagte sager har aldrig en afgørelsesdato, så de bruger i stedet dateOfDismissal; alle andre statusser bruger stadig dateOfDecision.",
      "Endnu ikke afgjorte sager har hverken en afgørelsesdato eller en henlæggelsesdato (sagen er stadig åben), så den serie er tæt på nul i alle år her — det er forventet, ikke en fejl.",
      "Et lille antal sager med en reel afgørelse mangler også en afgørelsesdato og bliver ikke reddet af et fallback, som henlagte sager er, så de optræder ikke i noget år i denne visning — i modsætning til indbringelsesdato-visningen dækker denne ikke alle sager i kildesystemet.",
    ],
    ...
    quarterlyStatusMethodLinesDecision: [
      "Opdelt efter kvartal for dateOfDecision (afgørelsesdato) i stedet — henlagte sager har aldrig en afgørelsesdato, så de bruger i stedet dateOfDismissal; alle andre statusser bruger stadig dateOfDecision.",
      "Endnu ikke afgjorte sager har hverken en afgørelsesdato eller en henlæggelsesdato (sagen er stadig åben), så den kolonne er tæt på nul i alle kvartaler her — det er forventet, ikke en fejl.",
      "Et lille antal sager med en reel afgørelse mangler også en afgørelsesdato og bliver ikke reddet af et fallback, som henlagte sager er, så de optræder ikke i noget kvartal i denne visning — i modsætning til indbringelsesdato-visningen dækker denne ikke alle sager i kildesystemet.",
    ],
